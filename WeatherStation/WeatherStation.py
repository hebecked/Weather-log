...
class weather_station(object):
    ...
    def get_raw_fixed_block(self, unbuffered=False):
        ...
        if unbuffered or not self.current_pos:
...
"""
    print ws.get_fixed_block(['date_time'],True)
    print ws.current_pos()
    print ws.get_fixed_block(['alarm'],True) 
    print ws.get_data(ws.current_pos())
    print ws.current_pos()
    print ws.get_fixed_block(['read_period'],True) , ws.current_pos()

    #print CUSBDrive.write_byte(ws.cusb, 43, 124)
    
    for data, ptr, logged in ws.live_data():
        print data

"""
#print ws.get_data(ws.current_pos(), True)